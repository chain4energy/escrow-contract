...
#[test]
fn test_set_load_timeout_success() {
    let app = App::default();
    ...
    assert_eq!(res.events[1].ty, "wasm-set_load_timeout");
...
#[test]
fn test_set_load_timeout_unauthorized() {
    let app = App::default();
    let escrow_code_id = CodeId::store_code(&app);
    ...
    let new_timeout = 120000; // 120 seconds in milliseconds
    let res = escrow_contract.set_load_timeout(new_timeout).call(&unauthorized_user);
