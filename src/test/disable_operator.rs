...
#[test]
fn test_disable_operator_success() {
    let app = App::default();
    ...
    // Verify the "disable_operator" event
    let disable_event = &res.events[1];
    assert_eq!(disable_event.ty, "wasm-disable_operator");
    assert_eq!(disable_event.attributes[0].key, "_contract_address");
    assert_eq!(
        disable_event.attributes[0].value,
        escrow_contract.contract_addr.to_string()
    );
    assert_eq!(disable_event.attributes[1].key, "operator_id");
    assert_eq!(disable_event.attributes[1].value, operator_id);
...
#[test]
fn test_disable_operator_not_found() {
    let app = App::default();
    let escrow_code_id = CodeId::store_code(&app);
    let did_code_id = DidContractCodeId::store_code(&app);
    ...
    // Attempt to disable a non-existent operator
    let res = escrow_contract
        .disable_operator("non_existent_operator".to_string())
        .call(&owner);
...
#[test]
fn test_disable_operator_unauthorized() {
    let app = App::default();
    let escrow_code_id = CodeId::store_code(&app);
    let did_code_id = DidContractCodeId::store_code(&app);
    ...
    // Attempt to disable the operator by an unauthorized user
